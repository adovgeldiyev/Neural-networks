"""
Azat Dovgeldiyev
CSC 578 HW5-Backprop hyperparameters
"""
...
class ReLu(object):
    ...
    @classmethod
    def derivative(cls, z):
        res = np.array([float(n > 0) for n in z])
...
class Network(object):
    ...
    def set_parameters(self, cost=QuadraticCost, act_hidden=Sigmoid,
        ...
        if self.dropoutpercent >0.0:
            self.dropout_mask = [np.zeros((i,1)) for i in self.sizes]

        #the only cost function for tanh is quadratic cost
        if (act_output == Tanh and cost != QuadraticCost):
            self.cost = QuadraticCost
            print("Tanh only accepts QuadraticCost function. Changing to Quadratic Cost..")
    ...
    def update_mini_batch(self, mini_batch, eta, lmbda, n):
        ...
        if self.regularization =='L1':
            #create a list to hold values for sign function
            sgn = []
            for i in self.weights:
                ...
                s[s<0] = int(-1)
                sgn.append(s)
            self.weights = [w - (eta*(lmbda/n))*s - (eta/len(mini_batch))*nw 
                        for w, nw, s in zip(self.weights, nabla_w, sgn)]
        ...
        for b, w in zip(self.biases, self.weights):
            ...
            if count < len(self.biases)-1:
                ...
                if self.dropoutpercent > 0:
                    activation=activation * self.dropout_mask[count-2]
                count+=1
